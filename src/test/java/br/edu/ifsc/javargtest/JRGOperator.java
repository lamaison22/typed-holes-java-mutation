...
public class JRGOperator {
    ...
    public Arbitrary<BinaryExpr> genLogiExpression(Map<String,String> ctx) {
        ...
        Arbitrary<Expression> e = mCore.genExpression(ctx, 
                PrimitiveType.booleanType());
                      
        Arbitrary<Expression> ex = mCore.genExpression(ctx, 
                PrimitiveType.booleanType());
                 
        return e.map(exp -> new BinaryExpr(
                e.sample(), ex.sample(), genLogiOperator().sample())
        );
    ...
    @Provide
    public Arbitrary<BinaryExpr> genArithBooleanFor(Map<String,String> ctx, VariableDeclarator var, Arbitrary<LiteralExpr> ex ) {
         Arbitrary<PrimitiveType.Primitive> t = mBase.primitiveTypesMatematicos();
        String tp = t.sample().toString();
        
        Arbitrary<Expression> e = mCore.genExpression(ctx, ReflectParserTranslator.reflectToParserType(tp));   
                 
        return e.map(exp -> new BinaryExpr(var.getNameAsExpression(), ex.sample(), genArithOperatorFor().sample()));
    ...
    public Arbitrary<BinaryExpr.Operator> genArithOperatorFor() {
        ...
        return Arbitraries.of(BinaryExpr.Operator.PLUS, BinaryExpr.Operator.MULTIPLY);
