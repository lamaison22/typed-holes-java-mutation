...
public class JRGStmt {
   ...
   @Provide
    public  Arbitrary<Statement> genStatement(Map<String,String> ctx) {
        ...
        try {
            if(mFuel > 0) {
                ...
                Arbitrary<Integer> s = Arbitraries.of(IF_STMT, VAR_DECL_STMT, VAR_DECLARATION_STMT);
                switch(s.sample()){
    ...
    public Arbitrary<IfStmt> genIfStmt(Map<String,String> ctx) {
        ...
        Arbitrary<Expression> e = mCore.genExpressionOperator(ctx,
                PrimitiveType.booleanType());       
       
        JRGLog.showMessage(JRGLog.Severity.MSG_XDEBUG, "genIfStmt::fim"); 
     
        return e.map(exp -> new IfStmt(exp, genBlockStmt(newCtxIf).sample(),
    ...
    public Arbitrary<ForStmt> genForStmt(Map<String,String> ctx) throws ClassNotFoundException {
        ...
        Arbitrary<LiteralExpr> intAll = Arbitraries.integers().between(25, 100).map(i -> new IntegerLiteralExpr(
                        String.valueOf(i)));
        
        Arbitrary<LiteralExpr> intLimited = Arbitraries.integers().between(1, 5).map(i -> new IntegerLiteralExpr(
                        String.valueOf(i)));
        ...
        Arbitrary<Expression> compare = mCore.genExpressionOperatorFor(newCtx, PrimitiveType.intType(),variable.getVariable(0),intAll );
        ...
        Arbitrary<Expression> atualiza = mCore.genExpressionMatchOperatorFor(newCtx, PrimitiveType.intType(),variable.getVariable(0),intLimited );
        
        NodeList<Expression> nodesAtu = new NodeList<>(atualiza.sample());
        
        JRGLog.showMessage(JRGLog.Severity.MSG_XDEBUG, "genForStmt::fim"); 
        return e.map(exp -> new ForStmt(nodes, compare.sample(), nodesAtu,a.sample()));
    }   
    ...
    public Arbitrary<VariableDeclarationExpr> genVarDeclAssignInt(Map<String,String> ctx) throws ClassNotFoundException {
        ...
        Arbitrary<LiteralExpr> inteiro = Arbitraries.integers().between(1, 10).map(i -> new IntegerLiteralExpr(
                        String.valueOf(i)));
    ...
    @Provide
    public List<Statement> genList(Map<String,String> ctx) throws ClassNotFoundException {
        JRGLog.showMessage(JRGLog.Severity.MSG_XDEBUG, "genVarDeclarator::inicio");
